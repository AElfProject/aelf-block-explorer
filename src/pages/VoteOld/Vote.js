/**
 * @file
 * @author huangzongzhe  zhouminghui yangpeiyang
 * 233333
...
export default class VotePage extends Component {
    ...
    componentDidMount() {
        ...
        getContractAddress().then(result => {
            ...
            aelf.chain.contractAt(result.electionContractAddr, result.wallet, (error, result) => {
                console.log('electionContract', result);
                this.setState({
                    electionContract: result
                });
            });

            aelf.chain.contractAt(result.voteContractAddr, result.wallet, (error, result) => {
                console.log('voteContract', result);
                this.setState({
                    voteContractAddr: result
                });
            });
        }).catch((e) => {
            message.error(e);
    ...
    render() {
        const {consensus, showDownloadPlugins, currentWallet, dividends, electionContract, voteContract, contracts, nightElf} = this.state;
