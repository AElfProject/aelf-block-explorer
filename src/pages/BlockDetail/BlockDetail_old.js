...
import { aelf, format, get, formatKey, getContractNames } from "../../utils";
...
class BlockDetailPage extends React.Component {
  ...
  fetchBlockInfo = async (input) => {
    ...
    this.setState({
      blockInfo: {
        ...
        txsCount:
          (result && result.Body && result.Body.TransactionsCount) ||
          "Not Found",
    ...
    if (
      (!txsList || !txsList.transactions || !txsList.transactions.length) &&
      blockHeight <= BestChainHeight + 6
    ) {
  ...
  componentDidUpdate() {
    ...
    const { blockHeight, txs_loading, blockHash } = this.state;
  ...
  handleTableChange = (pagination) => {
    ...
    this.getTxsList(blockInfo.blockHash, pagination.current - 1).then(
      (result) => {
        this.setState({
  ...
  renderTxsList() {
    const { txs, pagination, txs_loading } = this.state;
    return (
      <div>
  ...
  renderCol(key, value) {
    ...
    return (
      <Row key={key + Math.random()}>
        <Col xs={24} sm={24} md={6} lg={6} xl={6} className='title'>
          {formatKey(key)}
        </Col>
  ...
  renderExtra() {
    ...
    if (Object.keys(parsedResult).length > 0) {
      // todo: 从链获取并格式化数据
      const { miner, resources, tx_fee, dividends } = parsedResult;
      return [
        this.renderCol(
          "Miner",
        ...
        this.renderCol(
          "Transaction Fee",
          <Dividends dividends={JSON.parse(tx_fee)} />
        ),
        this.renderCol(
          "Resources Fee",
          <Dividends defaultSymbol='' dividends={JSON.parse(resources)} />
        ),
        this.renderCol(
          "Dividends",
          <Dividends dividends={JSON.parse(dividends)} />
        ),
      ];
    }
    return null;
