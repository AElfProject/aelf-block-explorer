...
const getOption = (tpsData, currentChain) => {
...
export default function TPSChart({ isMobile, data }: IProps) {
  ...
  return (
    <div className={clsx(clsPrefix, isMobile && `${clsPrefix}-mobile`)}>
      ...
      <ReactEchartComponent className="chart" option={getOption(data, currentChain)}></ReactEchartComponent>
    </div>
  );
}
