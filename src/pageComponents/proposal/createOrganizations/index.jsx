...
import { Button, Select, Tooltip, InputNumber, Input, Switch, message, Divider, Form, Modal } from 'antd';
import { useWebLogin, WebLoginContext } from 'aelf-web-login';
import constants, { API_PATH } from '@_store/common/constants';
import { commonFilter, getContractAddress, showTransactionResult, rand16Num } from '@_store/common/utils';
import { request } from '../../../../common/request';
import { getTokenList, getContract, sleep } from '../../../../common/utils';
import './index.less';
import { WebLoginInstance } from '../../../../utils/webLogin';
import { showAccountInfoSyncingModal } from '@_components/SimpleModal';
    ...
    rules: [
      ...
      },
      {
        validator(rule, value) {
          ...
          return value > 0
            ? Promise.resolve()
            : Promise.reject(new Error('Minimal Approval Threshold needs to be larger than 0'));
        },
      },
    ],
  },
...
const SELECT_OPTIONS_WITH_AUTHORITY = [proposalTypes.PARLIAMENT, proposalTypes.ASSOCIATION, proposalTypes.REFERENDUM];

const SELECT_OPTIONS_WITH_NO_AUTHORITY = [proposalTypes.ASSOCIATION, proposalTypes.REFERENDUM];
...
const CreateOrganization = () => {
  ...
  const INPUT_PROPS_MAP = useMemo(
    () => getInputPropsMap(formData.proposalType || proposalTypes.PARLIAMENT, formData.tokenSymbol, tokenList),
    ...
    <div className="create-organization">
      <div className="create-organization-header">
        ...
        <div className="create-organization-header-action">
          <Link to="/proposal/organizations">&lt;Back to Organization List</Link>
        </div>
      </div>
      <Divider />
      <Form form={form} initialValues={FORM_INITIAL} {...formItemLayout}>
        <FormItem label={FIELDS_MAP.proposalType.label} required {...FIELDS_MAP.proposalType}>
          <Select placeholder={FIELDS_MAP.proposalType.placeholder} onChange={handleProposalTypeChange}>
            {selectOptions.map((v) => (
        ...
        <ConditionSwitch>
          ...
          <Case condition={formData.proposalType === proposalTypes.REFERENDUM}>
            <FormItem label={FIELDS_MAP.tokenSymbol.label} required {...FIELDS_MAP.tokenSymbol}>
              <Select
                showSearch
        ...
        {formData.proposalType && formData.proposalType !== proposalTypes.PARLIAMENT ? (
          <FormItem label={FIELDS_MAP.proposers.label} {...FIELDS_MAP.proposers}>
            <TextArea placeholder={FIELDS_MAP.proposers.placeholder} autoSize />
          </FormItem>
        ) : null}
        <FormItem label={FIELDS_MAP.minimalApprovalThreshold.label} required {...FIELDS_MAP.minimalApprovalThreshold}>
          <InputNumber {...INPUT_PROPS_MAP} />
        </FormItem>
        <FormItem label={FIELDS_MAP.maximalRejectionThreshold.label} required {...FIELDS_MAP.maximalRejectionThreshold}>
          <InputNumber {...INPUT_PROPS_MAP} />
        </FormItem>
        <FormItem
        ...
        <FormItem label={FIELDS_MAP.minimalVoteThreshold.label} required {...FIELDS_MAP.minimalVoteThreshold}>
          <InputNumber {...INPUT_PROPS_MAP} />
        </FormItem>
        <FormItem {...tailFormItemLayout}>
          <Button shape="round" type="primary" loading={isLoading} onClick={handleSubmit}>
            Apply
          </Button>
        </FormItem>
      </Form>
