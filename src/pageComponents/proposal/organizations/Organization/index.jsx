...
import constants, { LOG_STATUS, organizationInfoPropTypes } from '@_store/common/constants';
import config from '../../../../../common/config';
import Circle from '../../../components/Circle';
import './index.less';
import { isPhoneCheck } from '../../../../../common/utils';
import { PRIMARY_COLOR } from '@_constants/style';
import addressFormat from '@_utils/formatter';
...
export function getCircleValues(proposalType, releaseThreshold, leftOrgInfo, bpCount = 1) {
  ...
  const { minimalApprovalThreshold, maximalRejectionThreshold, maximalAbstentionThreshold, minimalVoteThreshold } =
    releaseThreshold;
  let total;
  let coef = 1;
  let precision = 0;
...
export function getOrganizationLeftInfo(proposalType, leftOrgInfo, bpList, parliamentProposerList) {
  const { tokenSymbol, proposerAuthorityRequired, proposerWhiteList = {}, organizationMemberList = {} } = leftOrgInfo;
  let { proposers = [] } = proposerWhiteList;
...
const Organization = (props) => {
  ...
  const votesData = useMemo(
    () => getCircleValues(proposalType, releaseThreshold, leftOrgInfo, bpList.length),
    [proposalType, releaseThreshold, leftOrgInfo],
  );
  // console.log("votesData", votesData);
  const leftOrg = useMemo(
    () => getOrganizationLeftInfo(proposalType, leftOrgInfo, bpList, parliamentProposerList),
    [leftOrgInfo, bpList, parliamentProposerList],
  );
  // eslint-disable-next-line max-len
  const canEdit = useMemo(
    () => isProposer(logStatus, currentWallet.address, proposalType, leftOrgInfo, bpList, parliamentProposerList),
      ...
      <div className="organization-list-item gap-bottom">
        <Card title={<Title proposalType={proposalType} />}>
          <div className="organization-list-item-id">
            <div className="gap-right-large text-ellipsis">{addressFormat(orgAddress)}</div>
            {canEdit ? <EditOutlined color={PRIMARY_COLOR} onClick={handleEdit} /> : null}
          </div>
          <Divider />
          <div className="organization-list-item-info">
            <div className="organization-list-item-info-item">
              ...
              <span className="text-ellipsis">
                <a href={`${viewer.addressUrl}/${addressFormat(creator)}`} target="_blank" rel="noopener noreferrer">
          ...
          <div className="organization-list-item-votes">
            ...
            <Row gutter={16}>
              <Col span={12}>
                <div className="text-center organization-list-item-vote-desc">
            ...
            <Row gutter={16} className="organization-list-item-vote-chart">
              ...
              <Col span={8} offset={4}>
                <Circle className="organization-list-item-vote-chart-circle" type="Total" {...votesData.Total} />
              </Col>
            </Row>
            <Row gutter={16}>
              <Col span={12}>
              ...
              <Col span={12}>
                <div className="text-center organization-list-item-vote-desc">
                  ...
                  <div className="text-ellipsis" title={`${votesData.Total.num}(${votesData.Total.rate})`}>
