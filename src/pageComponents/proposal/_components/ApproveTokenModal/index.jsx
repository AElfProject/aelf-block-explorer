...
import { getContractAddress, getTxResult, sendTransactionWith } from '@_store/common/utils';
import constants from '@_store/common/constants';
import { showAccountInfoSyncingModal } from '@_components/SimpleModal';
import { getContract } from '@_constants/governance/utils';
import './index.less';
...
async function getProposalAllowanceInfo(aelf, proposalId, owner, symbol) {
  const [spender, decimals] = await Promise.all([getVirtualAddress(aelf, proposalId), getTokenDecimal(aelf, symbol)]);
...
const ApproveTokenModal = (props) => {
  const { action, aelf, onCancel, onConfirm, visible, wallet, tokenSymbol, proposalId, owner } = props;
  ...
  async function handleStake() {
    ...
    try {
      ...
      const result = await sendTransactionWith(callContract, getContractAddress('Token'), method, {
        spender,
        amount,
        symbol: tokenSymbol,
      });
  ...
  return (
    <Modal
      ...
      <Form form={form} className="approve-token-form" {...formItemLayout} onValuesChange={handleValueChange}>
