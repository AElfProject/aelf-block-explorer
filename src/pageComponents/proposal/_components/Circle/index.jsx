...
const circleTypes = [...Object.values(proposalActions), 'Total'];
...
const Circle = (props) => {
  const { isInProgress, value, threshold, type, maxValue, ...rest } = props;
  return (
    <If condition={isInProgress}>
      <Then>
        <CircularProgressbarWithChildren
          ...
          maxValue={maxValue}
          {...rest}>
          <CircularProgressbar value={value} minValue={0} styles={INNER_STYLE_MAP[type]} maxValue={maxValue} />
        </CircularProgressbarWithChildren>
      </Then>
      <Else>
        <CircularProgressbar value={value} minValue={0} styles={NORMAL_STYLE_MAP[type]} maxValue={maxValue} {...rest} />
      </Else>
    </If>
  );
};
