...
import { Tag, Button, Divider, PageHeader, Skeleton, Result, Row, Col, Tabs, Typography, Modal } from 'antd';
...
import { getBPCount, isPhoneCheck, sendHeight, validateURL } from '../../../../common/utils';
...
function CountDown(props) {
  ...
  const show = status !== proposalStatus.RELEASED && expired.isAfter(now) && expired.isBefore(threshold);
  return show ? <span className="warning-text">{`Expire ${now.to(expired)}`}</span> : null;
}
...
const ProposalDetail = () => {
  ...
  const send = async (action) => {
    if (proposalType === proposalTypes.REFERENDUM) {
      ...
      await sendTransactionWith(callContract, getContractAddress(proposalType), action, proposalId);
    }
  };
    ...
    <div className="proposal-detail">
      ...
      {info.loadingStatus === LOADING_STATUS.SUCCESS ? (
        <>
          ...
          <Divider />
          <Tabs type="card" activeKey={activeKey} onTabClick={(key) => changeTab(key)}>
