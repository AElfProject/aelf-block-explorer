...
import { Input, Button, Table, Pagination, message, Tag, Typography } from 'antd';
...
import constants, { API_PATH, LOG_STATUS, LOADING_STATUS, ACTIONS_COLOR_MAP } from '@_store/common/constants';
import { getContractAddress, sendTransactionWith } from '@_store/common/utils';
import './index.less';
...
const listColumn = [
  {
    ...
    width: 300,
    render: (voter) => (
      <a href={`${viewer.addressUrl}/${addressFormat(voter)}`} target="_blank" rel="noopener noreferrer">
  ...
  },
  {
    ...
    width: 300,
    render: (txId) => (
      <a href={`${viewer.txUrl}/${txId}`} target="_blank" rel="noopener noreferrer">
        {txId}
      </a>
    ),
  },
...
const VoteDetail = (props) => {
  const { proposalId, proposalType, logStatus, wallet, currentWallet, expiredTime, status, symbol } = props;
  ...
  return (
    <div className="vote-detail">
      <If
        condition={
          logStatus === LOG_STATUS.LOGGED && proposalType === proposalTypes.REFERENDUM && personVote.list.length > 0
        }>
        <Then>
          <div className="vote-detail-personal gap-bottom-small">
            ...
            <div className="vote-detail-personal-total gap-bottom">
              ...
              <Button className="gap-left" type="primary" disabled={!personVote.canReclaim} onClick={reclaimToken}>
                Reclaim
              </Button>
            </div>
            <TableLayer>
      ...
      <Search className="vote-detail-search" placeholder="Input voter address/transaction id" onSearch={onSearch} />
