...
import constants, { organizationInfoPropTypes } from '@_store/common/constants';
...
const OrganizationCard = (props) => {
  ...
  const thresholdValue = useMemo(
    () => getCircleValues(proposalType, releaseThreshold, leftOrgInfo, bpCount || bpList.length),
        ...
        <Col sm={12} xs={24}>
          <>
            <div
              ...
              <span className="sub-title gap-right-small">Minimal Approval Threshold:</span>
              <span className="text-ell">
                {thresholdValue[proposalActions.APPROVE].num}({thresholdValue[proposalActions.APPROVE].rate})
              </span>
            </div>
            <div
              className="gap-bottom-small text-ellipsis"
              title={`${thresholdValue[proposalActions.REJECT].num}(${thresholdValue[proposalActions.REJECT].rate})`}>
              <span className="sub-title gap-right-small">Maximal Rejection Threshold:</span>
              <span>
                {thresholdValue[proposalActions.REJECT].num}({thresholdValue[proposalActions.REJECT].rate})
              </span>
            </div>
            <div
              className="gap-bottom-small text-ellipsis"
              title={`${thresholdValue[proposalActions.ABSTAIN].num}(${thresholdValue[proposalActions.ABSTAIN].rate})`}>
              <span className="sub-title gap-right-small">Maximal Abstention Threshold:</span>
              <span>
                {thresholdValue[proposalActions.ABSTAIN].num}({thresholdValue[proposalActions.ABSTAIN].rate})
              </span>
            </div>
            <div
              className="gap-bottom-small text-ellipsis"
              title={`${thresholdValue.Total.num}(${thresholdValue.Total.rate})`}>
              <span className="sub-title gap-right-small">Minimal Vote Threshold:</span>
              <span>
