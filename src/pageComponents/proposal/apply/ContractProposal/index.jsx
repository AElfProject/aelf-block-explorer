...
import { Radio, Input, Button, Upload, Select, message, Tooltip, Form } from 'antd';
import { onlyOkModal } from '@components/SimpleModal/index.tsx';
import { useDispatch, useSelector } from 'react-redux';
import { API_PATH } from '@_store/common/constants';
import { destorySelectList, getProposalSelectListWrap } from '@redux/actions/proposalSelectList';
import { getContractAddress } from '@_store/common/utils';
...
const ContractProposal = (props) => {
  ...
  const contractFilter = (input) =>
    contractList.filter(({ contractName, address }) => contractName.indexOf(input) > -1 || address.indexOf(input) > -1)
      .length > 0;
  ...
  async function customValidateFields() {
    const [result] = await Promise.all([validateFields(), checkContractNameHandler(getFieldValue('name'))]);
    return result;
  }

  const isInWhiteList = async (author) => {
    if (CHAIN_ID !== 'AELF') {
      const list = await callGetMethodSend('Parliament', 'GetProposerWhiteList', '');
      // in white list
  ...
  const checkUpdateMode = async (address) => {
    try {
      const result = await callGetMethodSend('Genesis', 'GetContractInfo', address);
      const { author } = result;
      // Genesis contract ---> bpApproval
      // bp mode choose withoutApproval mode
      if (getContractAddress('Genesis') === author && approvalMode === 'withoutApproval') {
        message.error('Contract update failed. Please update this contract in BP Approval mode.');
        return false;
      }
      // withoutApproval mode choose bp mode
      if (getContractAddress('Genesis') !== author && (await isInWhiteList(author)) && approvalMode === 'bpApproval') {
        message.error('Contract update failed. Please update this contract in without Approval mode.');
        return false;
      }
      return true;
    } catch (e) {
  ...
  async function handleSubmit() {
    try {
      ...
      if (
        isUpdate &&
        !isUpdateName &&
        (approvalMode === 'withoutApproval' || contractMethod === 'ProposeNewContract') &&
      ...
      if (approvalMode === 'withoutApproval') {
        action = isUpdate ? 'UpdateUserSmartContract' : 'DeployUserSmartContract';
      } else {
  ...
  const contractAddressFormItem = () => {
    const list =
      approvalMode === 'withoutApproval' ? contractList.filter((ele) => !ele.isSystemContract) : contractList;
    return (
      <FormItem
  ...
  const contractNameFormItem = () => {
    return (
      <FormItem
        ...
        <Input disabled={isUpdate && currentContractInfo.isSystemContract} maxLength={150} />
      </FormItem>
    );
  };
  ...
  return (
    <div className="contract-proposal">
      <Form
        ...
        {approvalMode === 'bpApproval' && !(isUpdate && isUpdateName) ? contractMethodFormItem() : null}
        {approvalMode === 'withoutApproval' || contractMethodType.ProposeNewContract === contractMethod ? (
          <>
            {isUpdate ? contractAddressFormItem() : null}
            {!isUpdate || (isUpdate && isUpdateName) ? contractNameFormItem() : null}
