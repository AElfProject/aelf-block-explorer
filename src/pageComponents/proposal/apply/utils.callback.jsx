...
import { getContractAddress, getTxResult } from '@_store/common/utils';
...
export const useCallbackAssem = () => {
  ...
  const contractSend = useCallback(
    async (action, params, isOriginResult) => {
      ...
      throw new Error(result.error.message || (result.errorMessage || {}).message || 'Send transaction failed');
    },
    [wallet],
  );
...
export const useReleaseApprovedContractAction = () => {
  ...
  return useCallback(
    async (contract) => {
      ...
      if (!((result && +result.error === 0) || !result.error)) {
        isError = true;
        throw new Error((result.errorMessage || {}).message || result?.error?.message || 'Send transaction failed');
      }
...
export const useReleaseCodeCheckedContractAction = () => {
  ...
  return useCallback(
    async (contract, isDeploy) => {
      ...
      const proposalItem = proposalSelect.list.find((item) => item.proposalId === proposalId);
      if (!proposalItem) throw new Error('Please check if the proposalId is valid');
      const proposedContractInputHash = await getOriginProposedContractInputHash({
      ...
      let contractVersion = '';
      if (!isError) {
        const logs = await getDeserializeLog(aelf, txsId, ['ContractDeployed', 'CodeUpdated']);
        const { address } = logs ?? {};
      ...
      if (contractAddress) {
        if (!contractVersion) {
          const verRes = await callGetMethodSend('Genesis', 'GetContractInfo', contractAddress);
      ...
      return {
        ...
        children: (
          <div style={{ textAlign: 'left' }}>
            {!isError && contractAddress ? (
              <div>
                <AddressNameVer address={contractAddress} name={contractName || contract.name} ver={contractVersion} />
              </div>
            ) : (
