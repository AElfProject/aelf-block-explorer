...
import { addContractName, getTransactionResult, updateContractName } from '../../utils';
...
import { base64ToByteArray, byteArrayToHexString, hexStringToByteArray } from '../../../../utils/formater';
...
const CreateProposal = () => {
    ...
    return new Promise((resolve) => {
      try {
        interval(async () => {
          ...
          const proposalInfo = await callGetMethodSend('Parliament', 'GetProposal', {
          ...
          if (endTIme > proposalInfo?.expiredTime || endTIme - startTime > GET_CONTRACT_VERSION_TIMEOUT) {
  ...
  const openFailedWithoutApprovalModal = (isUpdate, transactionId) => {
    ...
    onOpenWithoutApprovalModal({
      ...
      message: (
        <div>
          <div>
            1. The contract code you deployed didn&apos;t pass the codecheck, possibly due to that it didn&apos;t
            implement methods in the ACS12 contract, etc.
          </div>
          <div>2. Method fee and size fee payment failed due to insufficient balance.</div>
        </div>
      ),
      transactionId,
    });
  ...
  const minedStatusWithoutApproval = async (name, txRes, isUpdate, address) => {
    try {
      ...
      if (isUpdate) {
        ...
        // get contractVersion
        const { contractVersion } = await callGetMethodSend('Genesis', 'GetContractInfo', address);
        const {
  ...
  async function submitContract(contract) {
    const { isUpdate, address, action, name, file, isOnlyUpdateName, onSuccess, contractMethod, approvalMode } =
      contract;
    let params = {};
    try {
      ...
      if (action === 'ProposeNewContract' || action === 'DeployUserSmartContract') {
        // deploy contract
      ...
      if (approvalMode === 'withoutApproval') {
        try {
          ...
          const byteArray = AElf.utils.sha256.array(base64ToByteArray(params.code));
          // get codeHash from code
          const codeHash = byteArrayToHexString(byteArray);
          const txRes = await getTransactionResult(
            aelf,
            result?.TransactionId || result?.result?.TransactionId || result.transactionId || '',
          );
          txRes.codeHash = codeHash;
          const { TransactionId: transactionId, Error: error, Status: status } = txRes;
          // if pre-check fail
          if (status === 'NODEVALIDATIONFAILED') {
            ...
            const minedRes = await minedStatusWithoutApproval(name, txRes, isUpdate, address);
            if (minedRes.status === 'success') {
              const { contractAddress, contractName, contractVersion } = minedRes;
              // open modal
              onOpenWithoutApprovalModal({
                isUpdate,
                status: {
                ...
                cancel: cancelWithoutApproval,
                message: <AddressNameVer address={contractAddress} name={contractName} ver={contractVersion} />,
              });
            } else {
              // exec fail modal
              onOpenWithoutApprovalModal({
                ...
                cancel: cancelWithoutApproval,
                message: 'This may be due to the failure in transaction which can be vviewed via Transaction ID:',
                transactionId,
              });
            }
          }
      ...
      const result = await contractSend(action, params);
      const txsId = result?.TransactionId || result?.result?.TransactionId || result.transactionId || '';
      if (!txsId) throw new Error('Transaction failed. Please reinitiate this step.');
      let Log;
      let txResult;
      if (result.data) {
        // portkey sdk login
      ...
      if (name && +name !== -1) {
        await addContractName(currentWallet, {
          contractName: name,
          txId: result?.TransactionId || result?.result?.TransactionId || result.transactionId || '',
      ...
      setApplyModal({
        ...
        children: (
          <div style={{ textAlign: 'left' }}>
            ...
            <CopylistItem label="Transaction ID" isParentHref value={txsId} href={`/tx/${txsId}`} />
          </div>
        ),
      });
    } catch (e) {
  ...
  return (
    <div className="proposal-apply">
      ...
      <WithoutApprovalModal open={withoutApprovalOpen} withoutApprovalProps={withoutApprovalProps} />
    </div>
  );
};
