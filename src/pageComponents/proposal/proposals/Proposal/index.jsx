...
} from '@pageComponents/proposal/common/constant';
import './index.less';
import VoteChart from '../../_components/VoteChart';
...
import { getBPCount } from '@_utils/gorvernance';
import ButtonWithLoginCheck from '@_components/ButtonWithLoginCheck';
...
const Title = (props) => {
  ...
  const showExpired =
    status !== proposalStatus.RELEASED && momentExpired.isAfter(now) && momentExpired.isBefore(threshold);
  return (
    <div className="proposal-list-item-title">
      ...
      {showExpired ? <span className="warning-text">{`Expire ${now.to(momentExpired)}`}</span> : null}
    </div>
  );
};
Title.propTypes = {
...
const Proposal = (props) => {
  ...
  return (
    <div className="proposal-list-item gap-bottom">
      <Card
        title={
          <Title status={status} expiredTime={expiredTime} proposalType={proposalType} votedStatus={votedStatus} />
        }>
        <div className="proposal-list-item-id">
          ...
          <div className="proposal-list-item-id-status">
            ...
            {status === proposalStatus.APPROVED && canRelease ? (
              ...
              <Button proposal-id={proposalId} type="link" size="small" onClick={handleRelease}>
                Release&gt;
              </Button>
            ) : null}
          </div>
        ...
        <div className="proposal-list-item-info">
          ...
          <div className="proposal-list-item-info-item">
            ...
            <span className="text-ellipsis">{addressFormat(contractAddress)}</span>
          </div>
          <div className="proposal-list-item-info-item">
