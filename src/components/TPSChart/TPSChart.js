...
export default class TPSChart extends React.Component {
    ...
    async getTpsData() {
        ...
        const startTime = endTime - 3600000 * 3;
        const result = await get(TPS_LIST_API_URL, {
    ...
    getOption() {
        ...
        const tpsList = this.state.tpsList;
        ...
        tpsList.map(item => {
            ...
            let mimutes = startTime.getMinutes();

            if ((mimutes + '').length === 1) {
                mimutes = '0' + mimutes;
            }

            xAixsData.push(`${hours}:${mimutes}`);
            seriesData.push(item.tpm);
        });
        seriesData[length - 1] = {
            value: seriesData[length - 1],
        ...
        // console.log(seriesData[length - 1]);
