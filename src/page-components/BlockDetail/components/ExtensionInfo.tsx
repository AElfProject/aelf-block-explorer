...
export default function ExtensionInfo({ extensionInfo }) {
  const renderObj = useMemo(
    () =>
      extensionInfo
        ? {
            'Block Size': `${(extensionInfo.blockSize || '0').toLocaleString()} Bytes`,
            'Merkle Tree Root Of Transactions': extensionInfo.merkleTreeRootOfTransactions,
            'Merkle Tree Root Of World State': extensionInfo.merkleTreeRootOfWorldState,
            'Merkle Tree Root Of Transaction State': extensionInfo.merkleTreeRootOfTransactionState,
