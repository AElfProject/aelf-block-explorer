...
require('./ResourceMoneyMarket.less');
...
export default class ResourceMoneyMarket extends PureComponent {
  ...
  render() {
    const { currentResourceSymbol, currentWallet, contracts, tokenConverterContract, tokenContract } = this.state;
    const { realTimeTransactionLoading, echartsLoading, nightElf } = this.state;
    const { account, onRefresh, endRefresh, appName, loginAndInsertKeypairs } = this.props;
    let loading = true;
