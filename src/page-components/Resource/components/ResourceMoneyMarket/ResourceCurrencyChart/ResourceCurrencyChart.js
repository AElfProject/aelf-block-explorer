...
function handleDataList(list = [], interval, length) {
  ...
  if (dates.length < length) {
    ...
    volumes = [...volumes, ...emptyList.map((_, i) => [i + volumes.length, '-', 0])];
...
class ResourceCurrencyChart extends PureComponent {
  ...
  getOption() {
    ...
    const { currentResourceType } = this.props;
    const { dates, volumes, prices } = handleDataList(list, this.intervalTimeList[buttonIndex], QUERY_RANGE);
    ...
    return (
      <div className="resource-currency-chart">
        ...
        <div className="resource-sub-container">
          ...
          <ReactEchartsCore echarts={echarts} option={this.getOption()} style={this.echartStyle} notMerge lazyUpdate />
        </div>
      </div>
    );
  }
