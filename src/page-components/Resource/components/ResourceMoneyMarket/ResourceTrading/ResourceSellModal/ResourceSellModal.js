...
import { SYMBOL, ELF_DECIMAL } from 'constants';
...
export default class ResourceSellModal extends PureComponent {
  ...
  getSellRes() {
    ...
    if (account.balance <= sellFee) {
      message.warning(`Your ${SYMBOL} balance is insufficient to pay the service charge.`);
      return;
    }
    const wallet = {
  ...
  render() {
    const { sellFee, SellELFValue, sellEstimateValueLoading, sellNum, currentResourceType } = this.props;
    ...
    return (
      <div className="modal resource-modal">
        <Row className="modal-form-item">
          ...
          <Col span={18} className="text-ellipse" title={`ELF_${currentWallet.address}_${CHAIN_ID}`}>
        ...
        <Row className="modal-form-item">
          ...
          <Col span={18}>
            <Spin spinning={sellEstimateValueLoading}>{thousandsCommaWithDecimal(SellELFValue)}</Spin>
          </Col>
        </Row>
