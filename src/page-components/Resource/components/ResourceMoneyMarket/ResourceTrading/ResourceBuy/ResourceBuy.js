...
import { Input, InputNumber, Slider, message, Spin, Button, Tooltip, Form } from 'antd';
...
import { SYMBOL, GENERAL_PRECISION } from 'constants/misc';
...
export default class ResourceBuy extends Component {
  ...
  getSlideMarks() {
    ...
    this.regionLine = [0, region, region * 2, region * 3, account.balance.toFixed(GENERAL_PRECISION)];
    const marks = {};
  ...
  getEstimatedElf(value) {
    ...
    value = +value;
    getEstimatedValueELF(currentResourceType, value, tokenConverterContract, tokenContract)
      .then((result) => {
  ...
  prepareParamsForEstimatedResource(elfAmount) {
    ...
    return getEstimatedValueRes(currentResourceType, elfAmount, tokenConverterContract, tokenContract, true);
  }
  ...
  getBuyModalShow() {
    ...
    const { currentWallet, contracts, toBuy, appName, nightElf, operateNumToSmall } = this.state;
    ...
    if (!regPos.test(buyNum) || buyNum === 0) {
      message.error(`${ONLY_POSITIVE_FLOAT_OR_INTEGER_TIP}${CHECK_BALANCE_TIP}`);
      this.setState({
        buyBtnLoading: false,
      });
      return;
  ...
  getSlideMarksHTML() {
    const { buyInputLoading, buyEstimateValueLoading, account, buyNum, buyElfValue } = this.props;
    ...
    return (
      <Tooltip title={BALANCE_LESS_THAN_OPERATE_LIMIT_TIP}>
        <Slider
          ...
          max={+(+balance - A_PARAM_TO_AVOID_THE_MAX_BUY_AMOUNT_LARGE_THAN_ELF_BALANCE).toFixed(GENERAL_PRECISION)}
          tipFormatter={disabled ? () => BALANCE_LESS_THAN_OPERATE_LIMIT_TIP : null}
        />
      </Tooltip>
    );
  }
  ...
  render() {
    const { buyNum, buyElfValue, buyInputLoading, buyEstimateValueLoading, account, currentResourceType } = this.props;
        ...
        <div className="trading">
          <div className="trading-input">
            <div className="resource-action-block">
              ...
              <Spin spinning={buyInputLoading} wrapperClassName="resource-action-input">
...
function getMax(inputMax) {
  const rawBuyNumMax = +(inputMax - A_PARAM_TO_AVOID_THE_MAX_BUY_AMOUNT_LARGE_THAN_ELF_BALANCE).toFixed(
    GENERAL_PRECISION,
  );
