...
import { SYMBOL, ELF_DECIMAL, BUY_MORE_THAN_HALT_OF_INVENTORY_TIP, FAILED_MESSAGE_DISPLAY_TIME } from 'constants';
import { thousandsCommaWithDecimal } from 'utils/formater';
import { regBuyTooManyResource } from 'utils/regExps';
import getStateJudgment from 'utils/getStateJudgment';
import { aelf } from 'utils/axios';
require('./ResourceBuyModal.less');
...
export default class ResourceBuyModal extends PureComponent {
              ...
              this.setState(
                ...
                () => {
                  if (regBuyTooManyResource.test(err.Error)) {
                    message.error(BUY_MORE_THAN_HALT_OF_INVENTORY_TIP, FAILED_MESSAGE_DISPLAY_TIME);
                    message.error(`Transaction id: ${transactionId}`, FAILED_MESSAGE_DISPLAY_TIME);
                    return;
                  }
                  message.error(
  ...
  render() {
    const { buyElfValue, buyNum, buyFee, buyInputLoading, buyEstimateValueLoading, currentResourceType } = this.props;
    ...
    return (
      <div className="modal resource-modal">
        <Row className="modal-form-item">
          ...
          <Col className="text-ellipse" span={18} title={`ELF_${currentWallet.address}_${CHAIN_ID}`}>
        ...
        <Row className="modal-form-item">
          ...
          <Col span={18}>
            <Spin spinning={buyInputLoading}>{thousandsCommaWithDecimal(buyNum)}</Spin>
          </Col>
        </Row>
        <Row className="modal-form-item">
          <Col span={6}>{SYMBOL}</Col>
          <Col span={18}>
            <Spin spinning={buyEstimateValueLoading}>{thousandsCommaWithDecimal(buyElfValue)}</Spin>
          </Col>
        </Row>
        <div className="service-charge">
          *Service Charge: {thousandsCommaWithDecimal(buyFee)} {SYMBOL}
        </div>
        <Button type="primary" className="modal-button buy-btn" onClick={this.getBuyRes.bind(this)} loading={loading}>
          Buy
        </Button>
        <div className="modal-tip">
          * To avoid price fluctuations leading to transaction failure, please complete the transaction within 30
          seconds.
        </div>
      </div>
    );
