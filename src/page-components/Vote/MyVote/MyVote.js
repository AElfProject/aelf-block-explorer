...
import { getAllTeamDesc, fetchPageableCandidateInformation } from 'constants/api';
import publicKeyToAddress from 'utils/publicKeyToAddress';
import { RANK_NOT_EXISTED_SYMBOL } from 'page-components/Vote/constants';
import { MY_VOTE_DATA_TIP } from 'constants/info';
...
export default class MyVote extends Component {
  ...
  processData(resArr) {
    ...
    const myVoteRecords = [...electorVotes.activeVotingRecords, ...electorVotes.withdrawnVotesRecords];
    ...
    myVoteRecords.forEach((record) => {
      const foundedNode = allNodeInfo.find((item) => item.candidateInformation.pubkey === record.candidate);
    ...
    withdrawableVoteAmount = withdrawableVoteRecords.reduce((total, current) => total + +current.amount, 0);
    console.log({
      myTotalVotesAmount,
      withdrawableVoteAmount,
    });
    this.processStatistData('myTotalVotesAmount', 'num', myTotalVotesAmount / ELF_DECIMAL);
    this.processStatistData('withdrawableVotesAmount', 'num', withdrawableVoteAmount / ELF_DECIMAL);
  ...
  processTableData(myVoteRecords, allTeamInfo) {
    ...
    tableData.forEach((record) => {
      ...
      if (record.isWithdrawn) {
        record.type = 'Redeem';
        record.operationTime = moment.unix(record.withdrawTimestamp.seconds).format('YYYY-MM-DD HH:mm:ss');
      } else if (record.isChangeTarget) {
        record.type = 'Switch Vote';
        record.operationTime = moment.unix(record.voteTimestamp.seconds).format('YYYY-MM-DD HH:mm:ss');
      } else {
        record.type = 'Vote';
        record.operationTime = moment.unix(record.voteTimestamp.seconds).format('YYYY-MM-DD HH:mm:ss');
      }
  ...
  render() {
    const { statistData, spinningLoading, tableData, currentWallet } = this.state;
