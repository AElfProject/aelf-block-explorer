...
function generateCandidateApplyForm() {
  ...
  return {
    formItems: [
      {
        label: 'Mortgage Add',
        render: <span className="list-item-value">{addressFormat(currentWallet.address)}</span>,
      },
      {
...
class CandidateApplyModal extends PureComponent {
  ...
  render() {
    ...
    const TooltipInput = forwardRef((props, ref) => {
      return (
        <>
          <Input ref={ref} {...props} placeholder="Please input admin address" />
          <Tooltip
    ...
    return (
      <Modal
        ...
        <Form {...modalFormItemLayout} ref={this.formRef}>
          ...
          <Form.Item label="Candidate Admin:" className="candidate-admin" name="admin" rules={rules}>
