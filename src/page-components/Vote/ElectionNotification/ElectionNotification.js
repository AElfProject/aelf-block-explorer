...
require('./ElectionNotification.style.less');
...
class ElectionNotification extends PureComponent {
  ...
  async componentDidMount() {
    const { changeVoteState, electionContract, multiTokenContract, dividendContract, profitContractFromExt } =
      this.props;
  ...
  async fetchData() {
    ...
    if (electionContract !== null && consensusContract !== null && dividendContract !== null && !this.hasRun) {
  ...
  quitElection() {
    const { currentWallet, electionContractFromExt, checkExtensionLockStatus, judgeCurrentUserIsCandidate } =
      this.props;
  ...
  render() {
    ...
    const { totalVotesAmount, showDownloadPlugin, statisData, statisDataLoading, applyModalVisible } = this.state;
