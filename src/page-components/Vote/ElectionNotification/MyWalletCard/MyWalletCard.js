...
require('./MyWalletCard.less');
import { thousandsCommaWithDecimal } from 'utils/formater';
import { ELF_DECIMAL, SYMBOL } from 'constants/misc';
...
export default class MyWalletCard extends PureComponent {
  ...
  // todo: combine
  componentDidMount() {
    const { changeVoteState, electionContract, multiTokenContract, profitContractFromExt } = this.props;
    if (electionContract && multiTokenContract && profitContractFromExt && !this.hasRun) {
      changeVoteState({
  ...
  fetchData(prevProps) {
    ...
    const { multiTokenContract, electionContract, shouldRefreshMyWallet, changeVoteState } = this.props;
    ...
    if (electionContract && multiTokenContract && activeVotedVotesAmount !== '-' && balance !== '-') {
            ...
            {isPhoneCheck() ? (
              <>
                ...
                <div>
                  ...
                  <span className="primary-color">{currentWallet.formattedAddress}</span>
                </div>
              </>
            ) : (
              <>
