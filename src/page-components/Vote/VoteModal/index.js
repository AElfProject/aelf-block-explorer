...
function disabledDate(current) {
  ...
  return (
    current && (current < moment().add(SHORTEST_LOCK_TIME, 'days').endOf('day') || current > moment().add(1080, 'd'))
  );
}
...
class VoteModal extends Component {
  ...
  getFormItems() {
    ...
    const switchVoteRecord = switchableVoteRecords.find((record) => record.key === switchVoteSelectedRowKeys[0]);
    ...
    return [
      {
        ...
        formItems: [
          ...
          },
          {
            ...
            tip: isIPhone() ? null : `Usable Balance: ${thousandsCommaWithDecimal(balance, false)} ${SYMBOL}`,
          },
          {
            label: 'Lock Time',
            render: (
  ...
  render() {
    const { voteModalVisible, handleVoteTypeChange, voteType, voteConfirmLoading, isLockTimeForTest, changeVoteState } =
      this.props;
      ...
      tipHTML = (
        <>
          <p className="tip-color">
            <div>
              Once the transfer is confirmed, your lock-up time will be reset. Another {formattedLockTime || 'days'}{' '}
              will be counted from today.
            </div>
        ...
        <Tabs
          ...
          {formItems.map((form, index) => (
            ...
            <TabPane
              ...
              key={form.type}>
              <Form ref={this.formRef} className="vote-modal-form" {...formItemLayout} onSubmit={this.handleSubmit}>
                {form.formItems.map((item) => (
                  // todo: there are repeat code in form

                  <Form.Item
                    required={item.label === 'Vote Amount' || item.label === 'Lock Time'}
                    label={item.label}
                    key={item.label}
