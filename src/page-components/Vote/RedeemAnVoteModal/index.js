...
import { NEED_PLUGIN_AUTHORIZE_TIP } from 'constants/info';
...
export class RedeemAnVoteModal extends PureComponent {
  generateVoteAnRedeemForm() {
    ...
    return {
      formItems: [
        {
          label: 'Node Name',
          render: <span className="form-item-value">{voteToRedeem.nodeName}</span>,
        },
        {
          label: 'Node Add',
          render: <span className="form-item-value">{voteToRedeem.nodeAddress}</span>,
        },
        {
          label: 'Redeem Amount',
  ...
  render() {
    const { redeemOneVoteModalVisible, changeVoteState, handleRedeemOneVoteConfirm } = this.props;
