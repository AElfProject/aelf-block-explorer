...
import { contractsNeedToLoad, FROM_WALLET, FROM_EXPIRED_VOTES, FROM_ACTIVE_VOTES, routePaths } from './constants';
...
class VoteContainer extends Component {
  constructor(props) {
    ...
    this.handleSwitchVoteSelectedRowChange = this.handleSwitchVoteSelectedRowChange.bind(this);
    this.handleVoteFromExpiredSelectedRowChange = this.handleVoteFromExpiredSelectedRowChange.bind(this);
    this.handleRedeemVoteSelectedRowChange = this.handleRedeemVoteSelectedRowChange.bind(this);
    ...
    this.handleRedeemOneVoteConfirm = this.handleRedeemOneVoteConfirm.bind(this);
  ...
  async componentDidMount() {
    ...
    try {
      ...
      contractsNeedToLoad.forEach((contractItem, index) => {
        this.getContractByContractAddress(result, contractItem.contractAddrValName, contractItem.contractNickname);
      });
    } catch (e) {
      // message.error(e);
      console.error(e);
  ...
  componentDidUpdate(prevProps, prevState) {
    const { shouldRefreshMyWallet, electionContract, currentWallet, shouldJudgeIsCurrentCandidate } = this.state;
  ...
  handleSwitchVoteSelectedRowChange(selectedRowKeys, selectedRows) {
    ...
    const switchVoteAmount = selectedRows.reduce((total, current) => total + +current.amount, 0);
    this.setState({
  ...
  handleVoteFromExpiredSelectedRowChange(selectedRowKeys, selectedRows) {
    console.log('selectedRows', selectedRows);
    console.log('selectedRowKeys changed: ', selectedRowKeys);
    const voteFromExpiredVoteAmount = selectedRows.reduce((total, current) => total + +current.amount, 0);
    this.setState({
    ...
    return new Promise((resolve, reject) => {
      getLogin(nightElf, getLoginPayload, (result) => {
        ...
        if (error === 0) {
          ...
          localStorage.removeItem('currentWallet');
          const message = error === 200010 ? 'Please Login.' : errorMessage.message;
  ...
  processDataVoteNeed(resArr) {
    ...
    const { activeVotingRecords, allVotedVotesAmount, activeVotedVotesAmount } = electorVote;
    ...
    [...switchableVoteRecords, ...withdrawableVoteRecords].forEach((record) => {
      ...
      const teamInfo = allTeamInfo.find((team) => team.public_key === record.candidate);
      ...
      record.formatedVoteTime = moment.unix(voteTimestamp.seconds).format('YYYY-MM-DD HH:mm:ss');
    });
    this.setState({
      expiredVotesAmount,
      activeVotingRecords,
  ...
  handleClick(e) {
    const ele = JSON.stringify(e.target.dataset) !== '{}' ? e.target : e.target.parentNode;
    const {
      role = 'default',
  ...
  handleRedeemOneVote(ele) {
    const { nodeaddress: nodeAddress, nodename: nodeName, amount, voteid } = ele.dataset;
  ...
  handleVoteClick(ele) {
    const { nodeaddress: nodeAddress, targetpublickey: targetPublicKey, nodename: nodeName = '' } = ele.dataset;
    this.getWalletBalance()
      .then((res) => {
        ...
        this.setState({
          ...
          nodeAddress,
          targetPublicKey,
          currentWalletName: JSON.parse(localStorage.getItem('currentWallet')).name,
          formattedBalance,
          nodeName,
        });
  ...
  handleRedeemClick(ele) {
    const { nodename: nodeName = 'Default', nodeaddress: nodeAddress, targetpublickey: targetPublicKey } = ele.dataset;
    ...
    electionContract.GetElectorVoteWithRecords.call({
      ...
      .then((res) => {
        ...
        redeemableVoteRecordsForOneCandidate.forEach((item) => {
          ...
          item.formatedVoteTime = moment.unix(item.voteTimestamp.seconds).format('YYYY-MM-DD HH:mm:ss');
  ...
  handleVoteFromWallet() {
    const { voteAmountInput, targetPublicKey, electionContractFromExt } = this.state;
  ...
  handleVoteFromExpiredVote() {
    ...
    const votesToRedeem = [];
    const sortedRedeemableVoteRecords = withdrawnableVoteRecords.sort((a, b) => b.amount - a.amount);
    ...
    for (let i = 0; i < sortedRedeemableVoteRecords.length; i++) {
      ...
      const totalAmount = votesToRedeem.reduce((total, current) => total + +current.amount, 0);
  ...
  checkTransactionResult(res, modalToClose) {
    const transactionId = res.result ? res.result.TransactionId : res.TransactionId;
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        console.log('transactionId', transactionId);
        aelf.chain.getTxResult(transactionId, (error, result) => {
          if (!result) {
            message.info("Temporaryly didn' get the transaction info. Please query the transaction later");
      ...
      .then((resArr) => {
        ...
        const dividendAmounts = schemeIds.map((item, index) => {
          ...
          value = Object.keys(value).reduce((acc, key) => {
            return {
              ...acc,
              [key]: new Decimal(value[key] || 0).dividedBy(`1e${decimals[key] || 8}`).toNumber(),
            };
          }, {});
          total = {
            ...total,
      ...
      <section className="vote-container vote-container-simple basic-container basic-container-white">
        <Menu
          ...
          <Menu.Item
            ...
            onClick={() => {
              this.props.router.push(routePaths.electionNotifi);
            }}>
          ...
          <Menu.Item
            ...
            onClick={() => {
              this.props.router.push(routePaths.myVote);
            }}>
            My Vote
          </Menu.Item>
        </Menu>
        ...
        <div>
          ...
          <section
            ...
            <VoteModal
              ...
              handleVoteTypeChange={this.handleVoteTypeChange}
              voteType={voteType}
              handleSwitchVoteSelectedRowChange={this.handleSwitchVoteSelectedRowChange}
              ...
              handleVoteFromExpiredSelectedRowChange={this.handleVoteFromExpiredSelectedRowChange}
              changeVoteState={this.changeVoteState}
            />
            ...
            <RedeemModal
              ...
              redeemableVoteRecordsForOneCandidate={redeemableVoteRecordsForOneCandidate}
              activeVoteRecordsForOneCandidate={activeVoteRecordsForOneCandidate}
              currentWallet={currentWallet}
              redeemVoteSelectedRowKeys={redeemVoteSelectedRowKeys}
              handleRedeemVoteSelectedRowChange={this.handleRedeemVoteSelectedRowChange}
              changeVoteState={this.changeVoteState}
            />
