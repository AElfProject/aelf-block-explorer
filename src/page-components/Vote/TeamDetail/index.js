...
import { getTeamDesc, fetchElectorVoteWithRecords, fetchCurrentMinerPubkeyList } from 'constants/api';
import { FROM_WALLET, A_NUMBER_LARGE_ENOUGH_TO_GET_ALL } from 'page-components/Vote/constants';
import publicKeyToAddress from 'utils/publicKeyToAddress';
import getCurrentWallet from 'utils/getCurrentWallet';
import { filterUserVoteRecordsForOneCandidate, computeUserRedeemableVoteAmountForOneCandidate } from 'utils/voteUtils';
require('./index.less');
...
class TeamDetail extends PureComponent {
  ...
  processAllCandidateInfo(allCandidateInfo) {
    const candidateVotesArr = allCandidateInfo.map((item) => item.obtainedVotesAmount).sort((a, b) => b - a);
    const currentCandidate = allCandidateInfo.find((item) => item.candidateInformation.pubkey === this.teamPubkey);
    ...
    const totalVoteAmount = candidateVotesArr.reduce((total, current) => total + +current, 0);
    ...
    const votedRate = totalVoteAmount === 0 ? 0 : ((100 * totalVotes) / totalVoteAmount).toFixed(2);
  ...
  fetchTheUsersActiveVoteRecords() {
    ...
    fetchElectorVoteWithRecords(electionContract, {
      ...
      .then((res) => {
        this.computeUserRedeemableVoteAmountForOneCandidate(res.activeVotingRecords);
      })
      .catch((err) => {
  ...
  computeUserRedeemableVoteAmountForOneCandidate(usersActiveVotingRecords) {
    ...
    const userRedeemableVoteAmountForOneCandidate =
      computeUserRedeemableVoteAmountForOneCandidate(userVoteRecordsForOneCandidate);
    this.setState({
  ...
  renderTopTeamInfo() {
    ...
    const { formattedAddress, isBP, userRedeemableVoteAmountForOneCandidate, hasAuth, data, isCandidate } = this.state;
          ...
          <Col md={18} sm={24} xs={24} className="card-container-left">
            <Row className={`${clsPrefix}-team-avatar-info`}>
              ...
              <Col className={`${clsPrefix}-team-info`} md={18} sm={18} xs={18}>
                ...
                <Paragraph ellipsis={{ rows: 1 }}>Location: {data.location || '-'}</Paragraph>
