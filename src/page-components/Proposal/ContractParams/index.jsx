...
import { DatePicker, Input, Checkbox, Button, Form } from 'antd';
import PropTypes from 'prop-types';
import { rand16Num, getParams } from '../../common/utils';
require('./index.less');
...
const RepeatedForm = (props) => {
  const { formKey, Comp, formOption, layout } = props;
    ...
    <div className="repeated-form">
      {list.map((v, i) => (
        <FormItem
          ...
          {...formOption}>
          {Comp}
          <Button onClick={() => deleteItem(i)} className="gap-left">
            X
          </Button>
        </FormItem>
      ))}
...
RepeatedForm.propTypes = {
  ...
  formOption: PropTypes.shape({
    initialValue: PropTypes.oneOfType([PropTypes.string, PropTypes.object, PropTypes.bool, PropTypes.number]),
...
const INPUT_TYPE_FROM_ITEM_MAP = {
  '.google.protobuf.Timestamp': {
    Comp: (props) => <DatePicker showTime placeholder="Please select time" {...props} />,
    form: {
      initialValue: moment(),
    },
  },
  bool: {
    Comp: (props) => <Checkbox {...props} />,
    form: {
      initialValue: false,
  ...
  return (
    <div className="proposal-contract-params gap-bottom-large">
      {Object.keys(flattedFormDescriptor).map((key) => {
        const { type, name, repeated } = flattedFormDescriptor[key];
        ...
        const { Comp, form } = item;
        if (repeated === true) {
          return <RepeatedForm key={key} formKey={key} layout={layout} name={name} Comp={<Comp />} formOption={form} />;
        }
        return (
          <FormItem key={key} label={key} {...layout}>
            <Comp />
          </FormItem>
        );
      })}
